...
export const analyzeMultipleCharts = action({
  ...
  handler: async (ctx, args) => {
    ...
    const requestedModel = args.aiModel || "deepseek/deepseek-chat-v3-0324:free";
    if (!validModels.includes(requestedModel)) {
      throw new Error(`Invalid AI model: ${requestedModel}. Valid models: ${validModels.join(', ')}`);
    }

    const model = args.isDemoMode ? "deepseek/deepseek-chat-v3-0324:free" : (args.aiModel || "deepseek/deepseek-chat-v3-0324:free");
