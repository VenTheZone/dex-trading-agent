...
export function useTrading() {
    ...
    const balanceRecordTimeout = useRef(null);
    // Paper trading engine instance (persisted across renders)
    const paperEngineRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const pollPositions = async () => {
            try {
                ...
                if (!keys?.hyperliquid.apiSecret || !keys?.hyperliquid.walletAddress)
                    return;
                const result = await pythonApi.getHyperliquidPositions(keys.hyperliquid.apiSecret, keys.hyperliquid.walletAddress, network === 'testnet');
                        ...
                        if (accountValue > 0) {
                            ...
                            if (marginUsagePercent >= 80 && isAutoTrading) {
                                if (!marginWarningShown.current) {
                                    toast.error(`⚠️ LIQUIDATION WARNING: ${marginUsagePercent.toFixed(1)}% margin usage! Auto-trading paused.`, { duration: 10000 });
                            ...
                            if (marginUsagePercent >= 60 && marginUsagePercent < 80 && !marginWarningShown.current) {
                                toast.warning(`⚠️ High margin usage: ${marginUsagePercent.toFixed(1)}%`, { duration: 5000 });
                            }
                        }
                    }
    ...
    const closePosition = useCallback(async (positionToClose) => {
        if (!positionToClose) {
    ...
    const closeAllPositions = async () => {
        try {
            ...
            const confirmed = window.confirm(`⚠️ Close all positions?\n\n` +
    ...
    const runAIAnalysis = async (symbol, currentPrice) => {
        try {
    ...
    const runMultiChartAIAnalysis = async (charts) => {
        ...
        try {
            ...
            const filteredCharts = charts.filter(chart => allowedCoins.length === 0 || allowedCoins.includes(chart.symbol));
    ...
    const executeTrade = async (symbol, action, side, price, size, stopLoss, takeProfit, reasoning, skipConfirmation = false) => {
        try {
            const allowedCoins = settings.allowedCoins || [];
            ...
            if (!leverageValidation.valid) {
                toast.error(`⚠️ Leverage too high for ${symbol}`, {
            ...
            if (action !== "close_position" && side) {
                ...
                if (!positionCheck.canOpen) {
                    toast.error(`⚠️ Position Rejected: ${positionCheck.reason}`, {
                ...
                if (riskData.riskLevel === 'danger' || riskData.riskLevel === 'critical') {
                    toast.warning(`⚠️ HIGH RISK POSITION: ${riskData.riskLevel.toUpperCase()}`, {
            ...
            if (mode === "live") {
                ...
                if (action === "close_position" && position) {
                    ...
                    // Open new position
                    const order = paperEngineRef.current.placeOrder(symbol, side === 'long' ? 'buy' : 'sell', size, price, 'market');
        ...
        const runAutoTrading = async () => {
            ...
            try {
                ...
                if (isDemoMode) {
                    ...
                    const validCharts = chartData.filter((chart) => chart !== null);
                    ...
                    if (validCharts.length < allowedCoins.length) {
                        const failedCoins = allowedCoins.filter(coin => !validCharts.find(chart => chart.symbol === coin));
                    ...
                    if (analysis.action === "open_long" || analysis.action === "open_short") {
                        ...
                        await executeTrade(recommendedSymbol, analysis.action, side, analysis.entryPrice, analysis.positionSize, analysis.stopLoss, analysis.takeProfit, `[DEMO] ${analysis.reasoning}`, true);
                    }
                ...
                if (analysis.action === "open_long" || analysis.action === "open_short") {
                    ...
                    await executeTrade(recommendedSymbol, analysis.action, side, analysis.entryPrice, analysis.positionSize, analysis.stopLoss, analysis.takeProfit, analysis.reasoning, true);
                }
                else if (analysis.action === "close" && position) {
                    toast.info(`📊 AI recommends closing ${position.symbol}`, {
                        description: `Confidence: ${analysis.confidence}%`,
                    });
                    await executeTrade(position.symbol, "close_position", position.side, position.currentPrice ?? position.entryPrice, position.size, undefined, undefined, analysis.reasoning, true);
                }
                else {
                ...
                if (!isActive)
                    return; // Don't show errors if component unmounted
