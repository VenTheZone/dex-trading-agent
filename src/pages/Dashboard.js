...
export default function Dashboard() {
    ...
    const liquidationRisk = position
        ...
        }, position.currentPrice || position.entryPrice, balance)
        : null;
    const handleModeToggle = () => {
        const newMode = mode === 'paper' ? 'live' : 'paper';
        if (newMode === 'live') {
            const confirmed = window.confirm('⚠️ WARNING: You are about to switch to LIVE TRADING mode.\\n\\n' +
    ...
    const handleNetworkToggle = () => {
        ...
        const confirmed = window.confirm(`Switch to Hyperliquid ${newNetwork.toUpperCase()}?\\n\\n` +
            (newNetwork === 'testnet'
    ...
    return (<div className="min-h-screen bg-black text-cyan-100">
      ...
      <div className="relative z-10">
        ...
        {/* Header */}
        <motion.header initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="border-b border-cyan-500/30 bg-black/80 backdrop-blur-sm">
          <div className="container mx-auto px-4 py-4 flex items-center justify-between">
            ...
            textShadow: '0 0 20px rgba(0,255,255,0.8), 2px 2px 0 #ff0080'
                  ...
                  {isAiThinking && (<motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center gap-2 bg-green-500/20 border border-green-500/50 px-3 py-1 rounded-full">
                ...
                <Breadcrumb>
                  <BreadcrumbList>
                    <BreadcrumbItem>
                      <BreadcrumbLink onClick={() => navigate('/')} className="text-cyan-400/70 hover:text-cyan-400 cursor-pointer font-mono text-sm">
                        Home
                      </BreadcrumbLink>
                    </BreadcrumbItem>
            ...
            <div className="flex items-center gap-4">
              ...
              <Badge variant="outline" className={`text-lg px-4 py-2 font-mono cursor-pointer ${network === 'mainnet'
            ? 'bg-purple-500/20 text-purple-400 border-purple-500'
            : 'bg-blue-500/20 text-blue-400 border-blue-500'}`} onClick={handleNetworkToggle}>
                <Network className="mr-2 h-4 w-4"/>
                {network === 'mainnet' ? '🟣 MAINNET' : '🔵 TESTNET'}
              </Badge>
              
              <Badge variant={mode === 'paper' ? 'secondary' : 'destructive'} className={`text-lg px-4 py-2 font-mono cursor-pointer ${mode === 'paper'
            ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500'
            : 'bg-red-500/20 text-red-400 border-red-500'}`} onClick={handleModeToggle}>
                {mode === 'paper' ? '📄 PAPER' : '🔴 LIVE'} TRADING
              </Badge>
              
              <Button variant="outline" size="icon" className="border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20" onClick={() => {
            const confirmed = window.confirm('⚠️ Clear all API keys and settings?\\n\\n' +
        ...
        <div className="container mx-auto px-4 py-6 space-y-6">
          ...
          {liquidationRisk && liquidationRisk.riskLevel !== 'safe' && (<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className={`p-4 rounded-lg border-2 ${liquidationRisk.riskLevel === 'critical'
                ? 'bg-red-500/20 border-red-500'
                : liquidationRisk.riskLevel === 'danger'
                    ? 'bg-orange-500/20 border-orange-500'
                    : 'bg-yellow-500/20 border-yellow-500'}`}>
              <div className="flex items-center gap-3">
                <AlertTriangle className={`h-6 w-6 ${liquidationRisk.riskLevel === 'critical' ? 'text-red-400 animate-pulse' :
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    {mode === 'demo' && (<Button variant="ghost" size="sm" onClick={() => {
                resetBalance();
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    {liquidationRisk && (<div className="mt-1">
                        <p className="text-xs font-mono text-gray-400">
                          Liq: ${liquidationRisk.liquidationPrice.toFixed(2)}
                        </p>
                        <div className={`text-xs font-mono font-bold ${liquidationRisk.riskLevel === 'safe' ? 'text-green-400' :
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex gap-2">
                      {position && (<>
                          <Button variant="ghost" size="sm" onClick={handleClosePosition} className="text-red-400 hover:text-red-300 hover:bg-red-500/20 font-mono text-xs">
                            <X className="h-4 w-4 mr-1"/>
                            Close
                          </Button>
                          <Button variant="ghost" size="sm" onClick={handleCloseAllPositions} className="text-red-400 hover:text-red-300 hover:bg-red-500/20 font-mono text-xs">
                            <X className="h-4 w-4 mr-1"/>
                            All
                          </Button>
                        </>)}
                      {!position && (<span className="text-sm text-gray-500 font-mono">No actions</span>)}
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {/* Charts - 2/3 width - Dynamically render based on allowed coins */}
            <div className={`lg:col-span-2 grid gap-4 ${settings.allowedCoins && settings.allowedCoins.length === 1
            ? 'grid-cols-1'
            : settings.allowedCoins && settings.allowedCoins.length === 2
                ...
                : settings.allowedCoins && settings.allowedCoins.length === 3
                    ...
                    : 'grid-cols-1 md:grid-cols-2'}`}>
              {settings.allowedCoins && settings.allowedCoins.length > 0 ? (settings.allowedCoins.map((symbol, index) => (<TradingChart key={`${symbol}-${index}`} symbol={symbol} chartId={index + 1}/>))) : (<div className="col-span-full flex items-center justify-center p-8">
            ...
            <div className="lg:col-span-1">
              <Tabs defaultValue="logs" className="w-full">
                <TabsList className="grid w-full grid-cols-3 bg-black/50">
                  <TabsTrigger value="logs" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono text-xs">
                    📊 Logs
                  </TabsTrigger>
                  <TabsTrigger value="news" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono text-xs">
                    📰 News
                  </TabsTrigger>
                  <TabsTrigger value="ai" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono text-xs">
                    🧠 AI
                  </TabsTrigger>
                </TabsList>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="fixed top-20 right-6 z-30">
          <Sheet modal={false}>
            <SheetTrigger asChild>
              <Button size="lg" className="bg-cyan-500 hover:bg-cyan-600 text-black font-bold font-mono shadow-[0_0_30px_rgba(0,255,255,0.6)] hover:shadow-[0_0_40px_rgba(0,255,255,0.8)] transition-all">
        ...
        <CloseAllPositionsDialog isOpen={showCloseAllDialog} onClose={() => setShowCloseAllDialog(false)} onConfirm={confirmCloseAllPositions} positionCount={position ? 1 : 0} mode={mode}/>
      </div>
    </div>);
}
