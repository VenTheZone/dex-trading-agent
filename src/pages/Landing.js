...
export default function Landing() {
    ...
    return (<div className="min-h-screen bg-black text-cyan-100">
      <TradingBackground />
      <UpdateNotification />
      
      <div className="relative z-10">
        {/* Hero Section */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className="min-h-screen flex flex-col items-center justify-center px-4">
          <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.8 }} className="text-center space-y-8 max-w-4xl">
            {/* Logo */}
            <motion.img src="/logo.png" alt="DeX Agent" width={120} height={120} className="mx-auto rounded-lg" style={{
            ...
            {/* Title */}
            <h1 className="text-6xl md:text-8xl font-bold text-cyan-400" style={{
            ...
            {/* CTA Buttons */}
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-8">
              <Button size="lg" onClick={handleNavigateToDashboard} className="text-xl px-8 py-6 bg-cyan-500 hover:bg-cyan-600 text-black font-bold font-mono shadow-[0_0_30px_rgba(0,255,255,0.8)] border-2 border-cyan-400">
                <Zap className="mr-2 h-6 w-6"/>
                ENTER DASHBOARD
              </Button>
              
              <Button size="lg" variant="outline" onClick={handleShowPreview} className="text-xl px-8 py-6 border-2 border-cyan-500 text-cyan-400 hover:bg-cyan-500/20 font-bold font-mono shadow-[0_0_20px_rgba(0,255,255,0.4)]">
                <Eye className="mr-2 h-6 w-6"/>
                PREVIEW DASHBOARD
              </Button>
              
              <Button size="lg" variant="outline" onClick={() => navigate('/docs')} className="text-xl px-8 py-6 border-2 border-purple-500 text-purple-400 hover:bg-purple-500/20 font-bold font-mono shadow-[0_0_20px_rgba(168,85,247,0.4)]">
                📚 DOCUMENTATION
              </Button>
            </motion.div>
          </motion.div>
          ...
          {/* Features Grid */}
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.8 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-24 max-w-6xl">
            {[
        ...
        ].map((feature, i) => (<motion.div key={i} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.9 + i * 0.1 }} className="bg-black/80 border border-cyan-500/50 rounded-lg p-6 text-center hover:border-cyan-500 transition-all hover:shadow-[0_0_30px_rgba(0,255,255,0.3)]">
          ...
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.2 }} className="mt-24 max-w-6xl w-full">
            <div className="flex items-center justify-center gap-3 mb-8">
              <h2 className="text-3xl font-bold text-cyan-400 font-mono text-center">
                🎯 Available Trading Pairs
              </h2>
              {!isInitialLoad && !globalError && (<Badge variant="outline" className="bg-green-500/20 text-green-400 border-green-500 font-mono animate-pulse">
                  🟢 LIVE
                </Badge>)}
            </div>

            {/* Global Error Alert */}
            {globalError && (<Alert variant="destructive" className="mb-6 bg-red-900/20 border-red-500/50">
            ...
            return (<motion.div key={token.symbol} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.3 + i * 0.05 }}>
                    <Card onClick={() => handleTokenClick(token)} className="bg-black/80 border-cyan-500/50 p-6 cursor-pointer hover:border-cyan-500 transition-all hover:shadow-[0_0_30px_rgba(0,255,255,0.3)] hover:scale-105">
                      ...
                      <div className="mt-4 pt-4 border-t border-cyan-500/30">
                        {isLoading ? (<div className="space-y-2">
                          ...
                          </div>) : hasError ? (<div className="space-y-2">
                        ...
                        retrySymbol(symbolKey);
                          ...
                          </div>) : liveData?.retryCount && liveData.retryCount > 0 ? (<div className="space-y-2">
                            <div className="text-xs text-yellow-400 font-mono mb-1 flex items-center gap-2">
                              <RefreshCw className="h-3 w-3 animate-spin"/>
                              {liveData.error}
                            </div>
                            {liveData.price > 0 && (<div className="text-sm text-gray-500 font-mono">
                                Last: ${liveData.price.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                        })}
                              </div>)}
                          </div>) : liveData?.price ? (<div>
          ...
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.5 }} className="mt-24 max-w-6xl w-full">
        ...
        ].map((feature, i) => (<motion.div key={i} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.6 + i * 0.05 }}>
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 2.0 }} className="mt-6">
          ...
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 2.2 }} className="mt-24 max-w-6xl w-full pb-16">
            <h2 className="text-3xl font-bold text-cyan-400 font-mono text-center mb-8">
              🚀 Getting Started Guide
            </h2>
            ...
            {/* Paper Trading Note */}
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.7 }} className="mt-6">
              <Card className="bg-gradient-to-br from-cyan-900/20 to-black/80 border-cyan-500/50 p-6 shadow-[0_0_30px_rgba(0,255,255,0.2)]">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-cyan-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
                    <span className="text-2xl">📄</span>
        ...
        <Dialog open={showPreview} onOpenChange={setShowPreview}>
          <DialogContent className="max-w-[95vw] max-h-[95vh] bg-black/95 border-cyan-500/50 p-0 overflow-hidden">
            ...
            <div className="relative w-full h-[80vh] overflow-auto custom-scrollbar">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="p-6">
            ...
            { label: 'Status', value: 'READY', icon: '⚡' }
        ].map((stat, i) => (<div key={i} className="bg-black/80 border border-cyan-500/50 rounded-lg p-4 shadow-[0_0_20px_rgba(0,255,255,0.2)]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {['BTC/USD', 'ETH/USD', 'SOL/USD', 'AVAX/USD'].map((symbol, i) => (<div key={i} className="bg-black/80 border border-cyan-500/30 rounded-lg p-4 h-64 flex items-center justify-center relative overflow-hidden">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {['BTC', 'ETH', 'SOL'].map((coin) => (<div key={coin} className="px-3 py-1 bg-cyan-500 text-black font-mono text-sm rounded shadow-[0_0_15px_rgba(0,255,255,0.4)]">
                              {coin}
                            </div>))}
                        </div>
                      </div>
        ...
        {/* Token Trading Modal */}
        <TokenTradingModal token={selectedToken} isOpen={showTokenModal} onClose={handleCloseTokenModal}/>
      </div>
    </div>);
}
