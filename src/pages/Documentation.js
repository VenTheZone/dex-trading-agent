...
import { ArrowLeft, Brain, TrendingUp, Shield, Zap, Database, Activity, GitBranch, CheckCircle, Info } from "lucide-react";
    ...
    return (<div className="min-h-screen bg-black text-cyan-100">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <Button variant="outline" onClick={() => navigate('/')} className="mb-4 border-cyan-500/50 text-cyan-400 hover:bg-cyan-500/20">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-300 text-sm leading-relaxed">
                      ...
                      <a href="https://nof1.ai/" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300 underline font-semibold">
                        Nof1's Alpha Arena
                      </a>
        ...
        <Tabs defaultValue="workflow" className="space-y-6">
          ...
          <TabsContent value="workflow" className="space-y-6">
              ...
              <CardContent>
                ...
                <div className="relative w-full h-[600px] bg-black/50 border border-cyan-500/30 rounded-lg overflow-hidden">
                  <svg className="absolute inset-0 w-full h-full">
                    {/* Draw connections */}
                    {workflowNodes.map((node) => node.connections.map((targetId) => {
            const target = workflowNodes.find((n) => n.id === targetId);
            if (!target)
                return null;
            return (<line key={`${node.id}-${targetId}`} x1={`${node.x}%`} y1={`${node.y}%`} x2={`${target.x}%`} y2={`${target.y}%`} stroke="rgba(0, 255, 255, 0.3)" strokeWidth="2" strokeDasharray="5,5"/>);
        }))}
                  </svg>
            ...
            return (<motion.div key={node.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.1 }} className="absolute" style={{
                    left: `${node.x}%`,
                    top: `${node.y}%`,
                    transform: 'translate(-50%, -50%)'
                }}>
                        <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} onClick={() => setSelectedNode(node.id)} className={`
                ...
                {selectedNode && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
                    <Card className="bg-gradient-to-br from-cyan-900/20 to-black/80 border-cyan-500/50">
                      ...
                      <CardContent className="text-gray-300 space-y-2">
                        ...
                        {selectedNode === "ai-analysis" && (<div className="mt-4 space-y-2">
                        ...
                        {selectedNode === "risk-check" && (<div className="mt-4 space-y-2">
                    ...
                    <Card className="bg-black/90 border-cyan-500/50 h-full hover:border-cyan-500 transition-all hover:shadow-[0_0_30px_rgba(0,255,255,0.3)]">
                      ...
                      <CardContent>
                        <ul className="space-y-2">
                          {feature.features.map((item, j) => (<li key={j} className="flex items-start gap-2 text-sm text-gray-300">
                              <CheckCircle className="h-4 w-4 text-green-400 mt-0.5 flex-shrink-0"/>
                              {item}
                            </li>))}
                        </ul>
        ...
        ].map((layer, i) => (<motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-start gap-4 p-4 bg-gradient-to-r from-purple-900/20 to-black/80 border border-purple-500/30 rounded-lg hover:border-purple-500/50 transition-all">
            ...
            },
            {
                ...
                warning: "Best for beginners"
        ...
        ].map((mode, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card className={`bg-black/90 border-${mode.color}-500/50 h-full hover:border-${mode.color}-500 transition-all hover:shadow-[0_0_30px_rgba(0,255,255,0.3)]`}>
                    ...
                    <CardContent className="space-y-4">
                      <ul className="space-y-2">
                        {mode.features.map((feature, j) => (<li key={j} className="flex items-start gap-2 text-sm text-gray-300">
        ...
        ].map((api, i) => (<motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-4 p-3 bg-black/50 border border-cyan-500/30 rounded font-mono text-sm hover:border-cyan-500/50 transition-all">
                      <Badge variant="outline" className={`${api.method === 'GET' ? 'text-green-400 border-green-500' : 'text-blue-400 border-blue-500'} font-bold`}>
                        {api.method}
                      </Badge>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-12 text-center">
          <Button size="lg" onClick={() => navigate('/dashboard')} className="bg-cyan-500 hover:bg-cyan-600 text-black font-bold font-mono shadow-[0_0_20px_rgba(0,255,255,0.5)]">
