 ...
 */
async function fetchFromHyperliquid(symbol) {
    try {
...
export async function fetchPriceWithFallback(symbol) {
    // Check cache first
    ...
    for (const baseUrl of API_CONFIG.BINANCE.BASE_URLS) {
        try {
            const response = await fetch(`${baseUrl}/ticker/price?symbol=${binanceSymbol}`, { signal: AbortSignal.timeout(API_CONFIG.BINANCE.TIMEOUT) });
            if (response.ok) {
                const data = await response.json();
    ...
    try {
        const response = await fetch(`${API_CONFIG.BINANCE.US_URL}/ticker/price?symbol=${binanceSymbol}`, { signal: AbortSignal.timeout(API_CONFIG.BINANCE.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
            if (data.code === undefined) {
                const price = parseFloat(data.price);
    ...
    // Try KuCoin
    try {
        const response = await fetch(`${API_CONFIG.KUCOIN.BASE_URL}?symbol=BTC-USDT`.replace('BTC-USDT', binanceSymbol.replace('USDT', '-USDT')), { signal: AbortSignal.timeout(API_CONFIG.KUCOIN.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
            if (data.data?.price) {
    ...
    // Try OKX
    try {
        const response = await fetch(`${API_CONFIG.OKX.BASE_URL}?instId=${binanceSymbol.replace('USDT', '-USDT')}`, { signal: AbortSignal.timeout(API_CONFIG.OKX.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
            if (data.data?.[0]?.last) {
    ...
    // Try Gate.io
    try {
        const response = await fetch(`${API_CONFIG.GATEIO.BASE_URL}?currency_pair=${binanceSymbol.replace('USDT', '_USDT')}`, { signal: AbortSignal.timeout(API_CONFIG.GATEIO.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
            if (data[0]?.last) {
    ...
    // Try MEXC
    try {
        const response = await fetch(`${API_CONFIG.MEXC.BASE_URL}?symbol=${binanceSymbol}`, { signal: AbortSignal.timeout(API_CONFIG.MEXC.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
            if (data.price) {
                const price = parseFloat(data.price);
    ...
    try {
        ...
        const response = await fetch(`${API_CONFIG.COINBASE.BASE_URL}/${coinbaseSymbol}/spot`, { signal: AbortSignal.timeout(API_CONFIG.COINBASE.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
            if (data.data?.amount) {
    ...
    try {
        ...
        const response = await fetch(`${API_CONFIG.KRAKEN.BASE_URL}?pair=${krakenSymbol}`, { signal: AbortSignal.timeout(API_CONFIG.KRAKEN.TIMEOUT) });
        if (response.ok) {
            const data = await response.json();
