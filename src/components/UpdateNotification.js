...
import { checkForUpdates, getUpdateInstructions } from "@/lib/version-checker";
...
export function UpdateNotification() {
    ...
    return (<AnimatePresence>
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md px-4">
        <Card className="bg-gradient-to-br from-cyan-500/20 to-purple-500/20 border-cyan-500/50 shadow-[0_0_30px_rgba(0,255,255,0.3)]">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="icon" onClick={() => setDismissed(true)} className="text-cyan-400 hover:bg-cyan-500/20">
                <X className="h-4 w-4"/>
              </Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            {versionInfo.releaseNotes && (<div className="text-xs text-gray-400 max-h-20 overflow-y-auto custom-scrollbar">
            ...
            <Button onClick={() => window.open(versionInfo.releaseUrl, "_blank")} className="w-full bg-cyan-500 hover:bg-cyan-600 text-black font-bold font-mono">
