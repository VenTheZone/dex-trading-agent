...
export function BalanceChart() {
    ...
    if (balanceLoading || !balanceHistory || balanceHistory.length === 0) {
        return (<Card className="bg-black/90 border-cyan-500/50">
        <CardHeader>
          ...
          <TabsContent value="balance" className="mt-4">
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={balanceChartData}>
                ...
                <YAxis stroke="#00ffff" style={{ fontSize: '12px', fontFamily: 'monospace' }} tickFormatter={(value) => `$${value.toLocaleString()}`}/>
                <Tooltip contentStyle={{
            backgroundColor: 'rgba(0, 0, 0, 0.9)',
            border: '1px solid rgba(0, 255, 255, 0.5)',
            borderRadius: '4px',
            fontFamily: 'monospace',
        }} labelStyle={{ color: '#00ffff' }} itemStyle={{ color: '#00ff00' }} formatter={(value) => [`$${value.toLocaleString()}`, 'Balance']}/>
                <Area type="monotone" dataKey="balance" stroke="#00ff00" strokeWidth={2} fill="url(#balanceGradient)"/>
              </AreaChart>
            </ResponsiveContainer>
          </TabsContent>
          
          <TabsContent value="pnl" className="mt-4">
            {positionLoading ? (<div className="text-center text-gray-500 py-8 font-mono">
                Loading position history...
              </div>) : pnlChartData.length === 0 ? (<div className="text-center text-gray-500 py-8 font-mono">
                No position history yet
              </div>) : (<ResponsiveContainer width="100%" height={300}>
                <LineChart data={pnlChartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0, 255, 255, 0.1)"/>
                  <XAxis dataKey="time" stroke="#00ffff" style={{ fontSize: '12px', fontFamily: 'monospace' }}/>
                  <YAxis stroke="#00ffff" style={{ fontSize: '12px', fontFamily: 'monospace' }} tickFormatter={(value) => `$${value.toFixed(2)}`}/>
                  <Tooltip contentStyle={{
                backgroundColor: 'rgba(0, 0, 0, 0.9)',
                border: '1px solid rgba(0, 255, 255, 0.5)',
                borderRadius: '4px',
                fontFamily: 'monospace',
            }} labelStyle={{ color: '#00ffff' }} formatter={(value, _name, props) => [
                `$${value.toFixed(2)}`,
                `P&L (${props.payload.symbol})`
            ]}/>
                  <Line type="monotone" dataKey="pnl" stroke="#ff0080" strokeWidth={2} dot={{ fill: '#00ffff', r: 3 }} activeDot={{ r: 5, fill: '#ff0080' }}/>
                </LineChart>
              </ResponsiveContainer>)}
          </TabsContent>
        </Tabs>
