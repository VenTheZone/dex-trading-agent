...
export function ApiKeySetup({ onComplete }) {
    const [keys, setKeys] = useState(storage.getApiKeys() || {
        hyperliquid: { apiKey: '', apiSecret: '', walletAddress: '' },
        openRouter: '',
    });
    const [, setMode] = useState('wallet');
    ...
    const handleDemoMode = () => {
        const balanceInput = document.getElementById('demo-initial-balance');
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto p-6">
      <Tabs defaultValue="wallet" className="w-full" onValueChange={(v) => setMode(v)}>
        <TabsList className="grid w-full grid-cols-3 bg-black/50 mb-6">
          <TabsTrigger value="wallet" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono">
            🔗 Wallet
          </TabsTrigger>
          <TabsTrigger value="api" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono">
            🔑 API Keys
          </TabsTrigger>
          <TabsTrigger value="demo" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono">
            🎮 Demo
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="api">
          <Card className="bg-black/90 border-cyan-500/50 shadow-[0_0_30px_rgba(0,255,255,0.3)]">
            ...
            {backendKeysConfigured && (<div className="px-6 pb-4">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input placeholder="0x1234...5678" value={keys.hyperliquid.apiKey} onChange={(e) => {
            ...
            setKeys({
                ...
                hyperliquid: { ...keys.hyperliquid, apiKey: sanitized }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="password" placeholder="0xabcd...ef01 (64-66 chars)" value={keys.hyperliquid.apiSecret} onChange={(e) => {
            ...
            setKeys({
                ...
                hyperliquid: { ...keys.hyperliquid, apiSecret: sanitized }
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      {keys.hyperliquid.apiSecret && (<p className={`text-xs font-mono ${keys.hyperliquid.apiSecret.trim().length === 64 || keys.hyperliquid.apiSecret.trim().length === 66
                ? 'text-green-400'
                : keys.hyperliquid.apiSecret.trim().length === 42
                    ? 'text-red-400 font-bold'
                    : 'text-red-400'}`}>
                          {keys.hyperliquid.apiSecret.trim().length} chars {keys.hyperliquid.apiSecret.trim().length === 64 || keys.hyperliquid.apiSecret.trim().length === 66
                ? '✅'
                : keys.hyperliquid.apiSecret.trim().length === 42
                    ? '❌ ADDRESS!'
                    : '❌'}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="password" placeholder="sk-or-v1-..." value={keys.openRouter} onChange={(e) => {
            ...
            setKeys({ ...keys, openRouter: sanitized });
              ...
              <Button onClick={handleSave} className="w-full bg-cyan-500 hover:bg-cyan-600 text-black font-bold font-mono shadow-[0_0_20px_rgba(0,255,255,0.5)] h-12 text-base">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input type="number" placeholder="10000" defaultValue="10000" min="100" max="1000000" step="1000" id="demo-initial-balance" className="bg-black/50 border-cyan-500/30 text-cyan-100 font-mono focus:border-cyan-500 h-12"/>
                  <p className="text-xs text-gray-400 font-mono">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="password" placeholder="sk-or-v1-... (optional for enhanced AI)" value={keys.openRouter} onChange={(e) => {
            const sanitized = sanitizeApiKey(e.target.value);
            setKeys({ ...keys, openRouter: sanitized });
        }} className="bg-black/50 border-cyan-500/30 text-cyan-100 font-mono focus:border-cyan-500 h-12"/>
                  <p className="text-xs text-gray-400 font-mono">
                ...
                <Button onClick={handleDemoMode} className="w-full bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-600 hover:to-purple-600 text-black font-bold font-mono shadow-[0_0_20px_rgba(0,255,255,0.5)] h-12 text-base">
