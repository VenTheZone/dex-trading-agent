...
export function TokenTradingModal({ token, isOpen, onClose }) {
        ...
        <div className="p-6 space-y-6 overflow-auto custom-scrollbar" style={{ maxHeight: 'calc(95vh - 100px)' }}>
          ...
          <Card className="bg-black/80 border-cyan-500/30 overflow-hidden">
            ...
            <div className="relative w-full" style={{ height: '600px' }}>
              {iframeError ? (<Alert className="m-4 bg-red-500/10 border-red-500/50">
                ...
                </Alert>) : (<iframe src={hyperliquidEmbedUrl} className="w-full h-full border-0" title={`${token.symbol} Trading Interface`} sandbox="allow-scripts allow-same-origin allow-forms" onError={handleIframeError}/>)}
            </div>
          </Card>

          {/* Chart Error Alert */}
          {chartError && (<Alert className="bg-yellow-500/10 border-yellow-500/50">
            ...
            <Tabs defaultValue="time" className="w-full">
              ...
              <TabsContent value="time" className="p-4 space-y-4">
                <div className="flex gap-2 mb-4">
                  {['5', '15', '60'].map((interval) => (<button key={interval} onClick={() => setTimeInterval(interval)} className={`px-4 py-2 rounded font-mono text-sm transition-all ${timeInterval === interval
              ...
              <TabsContent value="range" className="p-4 space-y-4">
                <div className="flex gap-2 mb-4">
                  {['10', '100', '1000'].map((range) => (<button key={range} onClick={() => setRangeInterval(range)} className={`px-4 py-2 rounded font-mono text-sm transition-all ${rangeInterval === range
                ? 'bg-cyan-500 text-black shadow-[0_0_20px_rgba(0,255,255,0.5)]'
                : 'bg-black/50 text-cyan-400 border border-cyan-500/30 hover:bg-cyan-500/20'}`}>
                      {range}R
                    </button>))}
...
function TimeChart({ symbol, interval, onError }) {
    useEffect(() => {
...
function RangeChart({ symbol, range, onError }) {
    useEffect(() => {
        const script = document.createElement('script');
        script.src = 'https://s3.tradingview.com/tv.js';
        script.async = true;
