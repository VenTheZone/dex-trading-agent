...
export function TradingControls() {
    ...
    const handleTestConnection = async () => {
        ...
        try {
            const result = await pythonApi.testHyperliquidConnection(network === 'testnet');
            if (result.success) {
    ...
    const handleAiModelChange = (model) => {
        if (model === 'qwen/qwen3-max') {
            const confirmed = window.confirm('⚠️ QWEN PRICING NOTICE\n\n' +
    ...
    return (<Card className="bg-gradient-to-br from-black/90 to-black/80 border-cyan-500/50 shadow-[0_0_30px_rgba(0,255,255,0.2)]">
      <CardHeader className="border-b border-cyan-500/30">
        <CardTitle className="text-cyan-400 font-mono flex items-center justify-between">
          ...
          <Button variant={isAutoTrading ? 'default' : 'outline'} size="sm" onClick={handleToggleAutoTrading} className={`font-mono font-bold transition-all ${isAutoTrading
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 pb-4 border-b border-cyan-500/30">
          <Label className="text-cyan-400 font-mono font-bold flex items-center gap-2">
            <Network className="h-4 w-4"/>
            Hyperliquid Connection
          </Label>
          <Button variant="outline" onClick={handleTestConnection} disabled={testingConnection} className="w-full border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 font-mono">
            {testingConnection ? (<>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 pb-4 border-b border-cyan-500/30">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs font-mono px-2 py-1 rounded ${aiModel === 'deepseek/deepseek-chat-v3-0324:free'
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant={aiModel === 'deepseek/deepseek-chat-v3-0324:free' ? 'default' : 'outline'} onClick={() => handleAiModelChange('deepseek/deepseek-chat-v3-0324:free')} className={`font-mono text-xs transition-all ${aiModel === 'deepseek/deepseek-chat-v3-0324:free'
            ? 'bg-cyan-500 text-black hover:bg-cyan-600 shadow-[0_0_15px_rgba(0,255,255,0.4)]'
            : 'border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20'}`}>
              DeepSeek
              <span className="ml-1 text-green-400">FREE</span>
            </Button>
            <Button variant={aiModel === 'qwen/qwen3-max' ? 'default' : 'outline'} onClick={() => handleAiModelChange('qwen/qwen3-max')} className={`font-mono text-xs transition-all ${aiModel === 'qwen/qwen3-max'
          ...
          {aiModel === 'qwen/qwen3-max' && (<Alert className="bg-yellow-500/10 border-yellow-500/50">
          ...
          {mode === 'paper' && (<Alert className="bg-cyan-500/10 border-cyan-500/50">
        ...
        {/* Position Management */}
        {position && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 pb-4 border-b border-cyan-500/30">
            ...
            <div className="bg-black/50 border border-cyan-500/30 rounded p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button variant="destructive" size="sm" onClick={handleClosePosition} className="font-mono bg-red-500 hover:bg-red-600">
                  <X className="mr-2 h-4 w-4"/>
                  Close Position
                </Button>
                <Button variant="outline" size="sm" onClick={handleCloseAllPositions} className="font-mono border-red-500/50 text-red-400 hover:bg-red-500/20">
                  <X className="mr-2 h-4 w-4"/>
                  Close All
                </Button>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 pb-4 border-b border-cyan-500/30">
          ...
          <div className="grid grid-cols-2 gap-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            ...
            return (<Button key={coin} variant={isSelected ? 'default' : 'outline'} size="sm" onClick={() => handleToggleCoin(coin)} className={`font-mono text-xs transition-all ${isSelected
                    ? 'bg-cyan-500 text-black hover:bg-cyan-600 shadow-[0_0_15px_rgba(0,255,255,0.4)]'
                    : 'border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20'} ${isMeme ? 'border-pink-500/50' : ''}`} title={isMeme ? 'Meme Coin - High Volatility' : ''}>
                  {isMeme && '🐕 '}
                  {coin.replace('USD', '')}
                </Button>);
        })}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-3">
          ...
          <Tabs value={chartType} onValueChange={(v) => setChartType(v)}>
            <TabsList className="grid w-full grid-cols-2 bg-black/50 border border-cyan-500/30">
              <TabsTrigger value="time" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono data-[state=active]:shadow-[0_0_15px_rgba(0,255,255,0.4)]">
                Time-Based
              </TabsTrigger>
              <TabsTrigger value="range" className="data-[state=active]:bg-cyan-500 data-[state=active]:text-black font-mono data-[state=active]:shadow-[0_0_15px_rgba(0,255,255,0.4)]">
                Range-Based
              </TabsTrigger>
            </TabsList>
          </Tabs>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(chartType === 'time' ? timeIntervals : rangeIntervals).map((interval) => (<Button key={interval} variant={chartInterval === interval ? 'default' : 'outline'} onClick={() => setChartInterval(interval)} className={`font-mono transition-all ${chartInterval === interval
                ? 'bg-cyan-500 text-black hover:bg-cyan-600 shadow-[0_0_15px_rgba(0,255,255,0.4)]'
                : 'border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20'}`}>
                {interval}
              </Button>))}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="pt-4 border-t border-cyan-500/30">
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="outline" className="w-full border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 font-mono">
            ...
            <SheetContent side="bottom" className="bg-black/95 border-cyan-500/50">
              ...
              <div className="space-y-4 mt-6">
                <div className="space-y-2">
                  ...
                  <Input type="range" min="1" max={localSettings.maxLeverage} step="1" value={localSettings.leverage} onChange={(e) => {
            ...
            setLocalSettings({
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" min="1" max="100" value={localSettings.maxLeverage} onChange={(e) => {
            ...
            setLocalSettings({
                ...
                <div className="flex items-center justify-between bg-black/50 p-3 rounded border border-cyan-500/30">
                  ...
                  <Switch checked={localSettings.allowAILeverage} onCheckedChange={(checked) => setLocalSettings({
            ...localSettings,
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-4 pt-4 border-t border-cyan-500/30">
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="outline" className="w-full border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 font-mono">
                <Sparkles className="mr-2 h-4 w-4"/>
            ...
            <SheetContent side="bottom" className="bg-black/95 border-cyan-500/50">
              ...
              <div className="space-y-4 mt-6">
                <div className="space-y-2">
                  ...
                  <Input type="number" value={localSettings.takeProfitPercent} onChange={(e) => {
            ...
            setLocalSettings({
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" value={localSettings.stopLossPercent} onChange={(e) => {
            ...
            setLocalSettings({
                ...
                <div className="flex items-center justify-between bg-black/50 p-3 rounded border border-cyan-500/30">
                  ...
                  <Switch checked={localSettings.useAdvancedStrategy} onCheckedChange={(checked) => setLocalSettings({
            ...localSettings,
            useAdvancedStrategy: checked
        })}/>
                </div>
                
                {localSettings.useAdvancedStrategy && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4 pl-4 border-l-2 border-cyan-500/50">
                    <div className="space-y-2">
                      <Label className="text-cyan-400 font-mono text-sm">Partial Profit (%)</Label>
                      <Input type="number" value={localSettings.partialProfitPercent} onChange={(e) => {
                ...
                setLocalSettings({
                    ...
                    <div className="flex items-center justify-between bg-black/50 p-3 rounded border border-cyan-500/30">
                      ...
                      <Switch checked={localSettings.useTrailingStop} onCheckedChange={(checked) => setLocalSettings({
                ...localSettings,
          ...
          <Button onClick={handleSaveSettings} className="w-full bg-cyan-500 hover:bg-cyan-600 text-black font-bold font-mono shadow-[0_0_20px_rgba(0,255,255,0.4)] transition-all">
            Apply Settings
          </Button>
        </motion.div>
        
        {/* AI Prompt Template Editor */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="space-y-4 pt-4 border-t border-cyan-500/30">
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="outline" className="w-full border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 font-mono">
            ...
            toast.info('Prompt reset to default template');
              ...
              <div className="space-y-4 mt-6">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={localPrompt} onChange={(e) => {
            const sanitized = sanitizeMultilineText(e.target.value, 10000);
            setLocalPrompt(sanitized);
        }} className="bg-black/50 border-cyan-500/30 text-cyan-100 font-mono min-h-[400px] text-sm" placeholder="Enter your custom AI trading strategy prompt..."/>
                  <p className="text-xs text-gray-500 font-mono">
